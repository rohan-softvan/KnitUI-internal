  ...
  render() {
    return this.props.width !== 'xs' ? (
      <div className="main-div">
        <Grid container>
          <Grid item xs={12} md={12} sm={12} lg={12} className="sidebar-ttl">
            <Tooltip title={this.state.projectTitle} placement="bottom-start">
              <Typography
                className={
                  this.props.selectedId != undefined && this.props.selectedId == 0 ? 'head-title-active' : 'head-title'
                }
                // onClick={() => {
          ...
          <Grid item xs={12} md={12} sm={12} lg={12} className="padd-lr-10">
            {this.state.menuList.map((tile, index) => (
              ...
              <Link
                ...
                to={tile.path}
                className={
                  this.props.selectedId && this.props.selectedId == tile.key ? 'menu-title-active' : 'menu-title'
                }
              >
                <ListItem key={tile.name} className="sidebar-title">
                  <img src={tile.imgLink.default} className="icons"></img>
                  <span className="title">{tile.name}</span>
                </ListItem>
              </Link>
